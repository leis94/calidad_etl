...
def dimensiones_clic_resueltos():
    ...
    df_dim_departamento = pd.DataFrame(
        (df_sql.loc[:, "DEPARTAMENTO", "SERVICIO_AFECTADO"].unique()), columns=["SERVICIO_AFECTADO"])

    df_dim_impacto = pd.DataFrame(
        (df_sql.loc[:, "IMPACTO", "URGENCIA"].unique()), columns=["IMPACTO", "URGENCIA"])

    df_dim_resolucion = pd.DataFrame(
        (df_sql.loc[:, "CODIGO_RESOLUCION", "METODO_RESOLUCION"].unique()), columns=["CODIGO_RESOLUCION", "METODO_RESOLUCION"])

    df_dim_tipo = pd.DataFrame(
        (df_sql.loc[:, "TIPO"].unique()), columns=["TIPO"])

    df_dim_dependencia = pd.DataFrame(
        (df_sql.loc[:, "DEPENDENCIA", "REGIONAL"].unique()), columns=["DEPENDENCIA", "REGIONAL"])

    df_dim_cumplimiento = pd.DataFrame(
        (df_sql.loc[:, "CUMPLIMIENTO"].unique()), columns=["CUMPLIMIENTO"])
    ...
    dict_dfs_several_columns = {"dim_departamento": ["DEPARTAMENTO", "SERVICIO_AFECTADO", df_dim_departamento],
                            ...
                            "dim_resolucion": ["CODIGO_RESOLUCION", "METODO_RESOLUCION", df_dim_resolucion],
